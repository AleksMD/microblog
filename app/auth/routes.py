...
from werkzeug.urls import url_parse #determines if the URL is relative or absolute
...
def login():
	...
	if current_user.is_authenticated:#handles if user logged in or not
		return redirect(url_for('main.index'))#if yes it redirects the latter to main page
...
def reset_password_request():
	...
	if form.validate_on_submit():
		...
		if user:
			send_password_reset_email(user) #sending email to user to accept reset password
